...
from revex.generators import ClassGenerator, Generator, GroupGenerator, OrGenerator, RepeatGenerator, StringGenerator
import string
    ...
    def parse(self, text: str = '') -> Generator:
        ...
        while current_index < len(text):
            ...
            if char  == '+':
                ...
                blocks[-1] = RepeatGenerator(blocks[-1], min_repeats=min_repeats, max_repeats=max_repeats)
    ...
    def parse_class(self, text: str, start_index: int) -> tuple[ClassGenerator, int]:
        ...
        while i < len(raw):
            if i + 2 < len(raw) and raw[i+1] == '-':
                start_char = raw[i]
                end_char = raw[i+2]
                if is_valid_range(start_char, end_char) and ord(start_char) <= ord(end_char):
                    characters.extend([chr(c) for c in range(ord(start_char), ord(end_char)+1)])
                    i += 3
                    continue
                else:
